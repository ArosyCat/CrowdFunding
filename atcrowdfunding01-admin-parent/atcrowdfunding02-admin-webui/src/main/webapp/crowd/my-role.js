// 声明专门的函数用来在分配Auth的模态框中显示Auth的树形结构的数据
function fillAuthTree() {
    ...
    for (var i = 0; i < authIdArray.length; i++) {
        ...
        var checked = true;
        // checkTypeFlag设置为false表示不联动，不联动是为了避免把不该勾选的勾选上
...
function getPageInfoRemote() {
    ...
    // 如果当前响应状态码不是200，说明发生了错误或者其他意外情况。显示提示消息，让当前函数停止执行
    if (statusCode != 200) {
...
function paginationCallBack(pageIndex,jQuery) {
    ...
    // 修改全局变量(window对象)的pageNum
    window.pageNum = pageIndex + 1;

    // 不太明白这里为什么不需要去拼接keyword和pageNum它也能保持查询条件进行分页
    // 例如：admin的分页是window.location.href = "admin/get/page.html?pageNum=" + pageNum + "&keyword=${param.keyword}";
    // 猜测也许是RoleHandler的getPageInfo()参数中一直带着？
    // 调用分页函数(不会造成递归死循环，原因是两个操作不是同步调用的，什么时候点页码什么时候才会调用generatePage())
    generatePage();
